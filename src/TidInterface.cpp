...
bool TidInterface::Attach(const std::string& pipe) {
	...
	this->pubset_->Add<cnbiros_bci::TidMessage>(this->rosnode_->getNamespace()+"/"+CNBIROS_BCI_TID_CNBI2ROS);
	
	// Create and subscribe to ROS topic
	this->subset_->Add(this->rosnode_->getNamespace()+"/"+CNBIROS_BCI_TID_ROS2CNBI, &TidInterface::callback_ros2tid, this);
	...
	while(this->rosnode_->ok()) {
		...
		for(auto it = this->tidclset_->Begin(); it != this->tidclset_->End(); ++it) {
			...
			if(it->second->GetMessage(&cnbiIds) == true) {
				...
				this->pubset_->Publish(this->rosnode_->getNamespace()+"/"+CNBIROS_BCI_TID_CNBI2ROS, rosIdm);
			}
		}
